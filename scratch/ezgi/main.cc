...
std::map<uint64_t, Time> SentTime;
...
{
    SentTime[packet->GetUid()] = Simulator::Now();
    packetsSent++;
}
...
{
    if (SentTime.find(packet->GetUid()) != SentTime.end())
    {
        Time sentTime = SentTime[packet->GetUid()];
...
int main(int argc, char *argv[])
{
    ...
    // Callback fonksiyonlarını ekle
    Config::ConnectWithoutContext("/NodeList/*/DeviceList/*/$ns3::LorawanMac/Tx", MakeCallback(&PacketSent));
    Config::ConnectWithoutContext("/NodeList/*/DeviceList/*/$ns3::LorawanMac/Rx", MakeCallback(&PacketReceived));
    ...
    // Performans metriklerini hesapla ve çıktıla
    double pdr = static_cast<double>(packetsReceived) / packetsSent;
    double avgDelay = static_cast<double>(totalDelay) / packetsReceived;
